...
-- update engine T¡ã, high T¡ã (bad condition engine door, heater...) can cause damage to the engine
function Vehicles.Update.Engine(vehicle, part, elapsedMinutes)
	...
	if vehicle:isEngineRunning() then
		...
		-- having no engine door or in bad condition will make the engine goes in high T¡ã
		if engineDoor then
	...
	-- stop updating parts when T¡ã reach 0
